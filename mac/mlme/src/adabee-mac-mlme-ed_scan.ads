...
is
   ...
   function Is_Valid_ED_SCAN_Req (Request : MLME_Request_Type) return Boolean
   with Global => null;
...
private
   ...
   is (not Req_SAP.Is_Null (Handle)
       and then Is_Valid_ED_SCAN_Req (Req_SAP.Request_Reference (Handle).all));
   ...
   function Is_Valid_ED_SCAN_Req_And_Cfm
     (Handle : AdaBee.MAC.MLME.Req_SAP.Service_Handle) return Boolean
   is (not Req_SAP.Is_Null (Handle)
       and then Req_SAP.Confirm_Written (Handle)
       and then
         Is_Valid_ED_SCAN_Req_And_Cfm
